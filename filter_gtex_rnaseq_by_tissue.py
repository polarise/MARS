...
example:
mkdir Whole_Blood
./script.py mapped_sample_names_by_tissue/Whole_Blood.txt GTEx_gene_rpkm.gct > Whole_Blood/GTEx_gene_rpkm.txt"""
		sys.exit( 0 )
	...
	for row in f:
		...
		cov = scipy.std( vals )/scipy.mean( vals )
		cov_thresh = 0
		if scipy.log( cov ) < cov_thresh:
			print >> sys.stderr, "Log of COV less than %s for %s" % ( cov_thresh, L[0] )
			continue
		"""

		# how to process row
		this_row = dict( zip( sample_names, L[2:] ))
